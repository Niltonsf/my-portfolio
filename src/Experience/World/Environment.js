...
export default class Environment {
	...
	setSunLight() {
		...
		if (this.debug.active) {			
			this.debugFolder.add(this.sunLight, 'intensity').min(0).max(10).step(0.001).name('sunLightIntesity');
	...
	setEnvironment() {
		...
		// Debug
		if (this.debug.active) {
			this.debugFolder.add(this.environmentMap, 'intesity').min(0).max(4).step(0.001).onChange(this.environmentMap.updateMaterials);
		}
	}
}